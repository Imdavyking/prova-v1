...
import (
	...
	"os"
	"strconv"
	expression "proof-generator/acir/expression"
...
func LoadACIRFromReader[T shr.ACIRField, E constraint.Element](r io.Reader) (ACIR[T, E], error) {
	var acir ACIR[T, E]
	...
	if err := dec.Decode(&acir); err != nil {
...
func LoadACIR[T shr.ACIRField, E constraint.Element](fileName string) (ACIR[T, E], error) {
	...
	return LoadACIRFromReader[T, E](file)
}

func LoadACIRFromJSON[T shr.ACIRField, E constraint.Element](data []byte) (ACIR[T, E], error) {
	return LoadACIRFromReader[T, E](bytes.NewReader(data))
}
