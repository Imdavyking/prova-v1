...
import (
	...
	"math/big"
	"os"
	hdr "proof-generator/acir/header"
	shr "proof-generator/acir/shared"
	"fmt"
...
func LoadWitnessStacksFromBytes[T shr.ACIRField](data []byte, modulus *big.Int) (WitnessStacks[T], error) {
...
func LoadWitnessStacksFromFile[T shr.ACIRField](filePath string, modulus *big.Int) (WitnessStacks[T], error) {
	...
	if err != nil {
		log.Error().Err(err).Str("file", filePath).Msg("Failed to read witness file")
		return WitnessStacks[T]{}, err
	}

	return LoadWitnessStacksFromBytes[T](data, modulus)
}

func (acir *ACIR[T, E]) GetWitnessFromBytes(data []byte, field *big.Int) (witness.Witness, error) {
	witnesses, err := LoadWitnessStacksFromBytes[T](data, field)
	if err != nil {
		return nil, fmt.Errorf("failed to load witness stack from bytes: %w", err)
	}
	return acir.buildWitness(witnesses, field)
}
...
func (acir *ACIR[T, E]) buildWitness(witnesses WitnessStacks[T], field *big.Int) (witness.Witness, error) {
